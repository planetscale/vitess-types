...
func (m *ExecuteOptions_AuthoritativeTimeout) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	i := len(dAtA)
	i = encodeVarint(dAtA, i, uint64(m.AuthoritativeTimeout))
	i--
	dAtA[i] = 0x1
	i--
	dAtA[i] = 0x88
...
var vtprotoPool_BoundQuery = sync.Pool{
	New: func() interface{} {
		return &BoundQuery{}
	},
}

func (m *BoundQuery) ResetVT() {
	m.Reset()
}
func (m *BoundQuery) ReturnToVTPool() {
	if m != nil {
		m.ResetVT()
		vtprotoPool_BoundQuery.Put(m)
	}
}
func BoundQueryFromVTPool() *BoundQuery {
	return vtprotoPool_BoundQuery.Get().(*BoundQuery)
}
...
func (m *ExecuteOptions_AuthoritativeTimeout) SizeVT() (n int) {
	...
	var l int
	_ = l
	n += 2 + sov(uint64(m.AuthoritativeTimeout))
	return n
}
	...
	for iNdEx < l {
		...
		case 3:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.TabletType |= dev.TabletType(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
...
func (m *ExecuteRequest) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			...
			if m.EffectiveCallerId == nil {
				m.EffectiveCallerId = &dev1.CallerID{}
			}
